            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                if (actionID is SplitShot or HeatedSplitShot)
                {
                    ...
                    if (!inCombat)
                    {
                        ...
                        if (GetRemainingCharges(Reassemble) < GetMaxCharges(Reassemble) && IsOffCooldown(AirAnchor) && level >= Levels.AirAnchor)  //dsssq:（>=lv.76）机工起手，整备ing_空气锚
                            return AirAnchor;
                    }

                    ///<summary>机工起手：进战</summary>
                    ///<author>dsssq</author>
                    if (inCombat && !openerFinished)
                    {
                        if (subStep == 0)//【抢开情况】run 整备
                        {
                            if (GetRemainingCharges(Reassemble) < GetMaxCharges(Reassemble)) subStep++;
                            else return Reassemble;
                        }
                        if (subStep == 1)//【抢开情况】run 空气锚（>=lv.76）
                        {
                            if (IsOnCooldown(AirAnchor) || level < Levels.AirAnchor) subStep++;
                            else return AirAnchor;
                        }
                        if (subStep == 2)
                        {
                            if (GetRemainingCharges(Ricochet) < GetMaxCharges(Ricochet)) subStep++;
                            else return Ricochet;
                        }
                        if (subStep == 3)
                        {
                            if (GetRemainingCharges(GaussRound) < GetMaxCharges(GaussRound)) subStep++;
                            else return GaussRound;
                        }
                        if (subStep == 4)
                        {
                            if (IsOnCooldown(Drill) || level < Levels.Drill) subStep++;
                            else return Drill;
                        }
                        if (subStep == 5)
                        {
                            if (IsOnCooldown(BarrelStabilizer) || level < Levels.BarrelStabilizer) subStep++;
                        ...
                        if (subStep == 7)
                        {
                            if (lastComboMove == SlugShot) subStep++;
                            else return OriginalHook(SlugShot);
                        }
                        if (subStep == 8)
                        {
                            if (GetRemainingCharges(Ricochet) < GetMaxCharges(Ricochet) - 1) subStep++;
                            else return Ricochet;
                        }
                        if (subStep == 9)
                        {
                            if (GetRemainingCharges(GaussRound) < GetMaxCharges(GaussRound) - 1) subStep++;
                            else return GaussRound;
                        }
                        if (subStep == 10)
                        {
                            if (lastComboMove == CleanShot) subStep++;
                            else return OriginalHook(CleanShot);
                        }
                        if (subStep == 11)
                        {
                            if (GetRemainingCharges(Reassemble) == 0) subStep++;
                            else return Reassemble;
                        }
                        if (subStep == 12)
                        {
                            if (IsOnCooldown(Wildfire) || level < Levels.Wildfire) subStep++;
                            else if (CanDelayedWeave(actionID)) return Wildfire;
                        }
                        if (level >= Levels.ChainSaw)//分支：>=lv.90
                        {
                            if (subStep == 13)
                            {
                                if (IsOnCooldown(ChainSaw)) subStep++;
                                else return ChainSaw;
                            }
                            if (subStep == 14)
                            {
                                if (gauge.Battery == 0) subStep++;
