...
namespace XIVSlothCombo
{
    ...
    public sealed partial class XIVSlothCombo : IDalamudPlugin
    {
        ...
        public XIVSlothCombo(DalamudPluginInterface pluginInterface)
        {
            ...
            Service.CommandManager.AddHandler(Command, new CommandInfo(OnCommand) { HelpMessage = "Open a window to edit custom combo settings.", ShowInHelp = true, });
...
// #endif
        ...
        private void PrintMotD()
        {
            try
            {
                ...
                List<Payload>? payloads = new() { starterMotd, EmphasisItalicPayload.ItalicsOn, new TextPayload(data.Trim()), EmphasisItalicPayload.ItalicsOff };

                Service.ChatGui.PrintChat(new XivChatEntry { Message = new SeString(payloads), Type = XivChatType.Echo });
            }
...
// #endif
        ...
        private static void UpgradeConfig4()
        {
            ...
            Service.Configuration.EnabledActions = Service.Configuration.EnabledActions4.Select(preset => (int)preset switch
            {
                ...
                77 => 2404,
                _ => 0,
            }).Where(id => id != 0).Select(id => (CustomComboPreset)id).ToHashSet();
